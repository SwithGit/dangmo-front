...
export function RegionInsightPanel({ result, announcement, onWrite }: {
  ...
  return <section className="dm-region-insight-result" aria-live="polite">
    <header className="dm-region-insight-head">
      <div><span>지역 인사이트 분석</span><h2>{insight.analysisUnit.name} × {insight.announcement.title}</h2><p>{insight.summary}</p></div>
    ...
    <div className="dm-region-analysis-meta">
      <span><small>분석 단위</small>{insight.analysisUnit.kind === "metro" ? "광역시 전체" : "시·군"}</span>
    ...
    {blocked ? <div className="dm-region-empty">
      ...
      {insight.readiness.missing.length > 0 && <ul>{insight.readiness.missing.map((item) => <li key={item}>{item}</li>)}</ul>}
      <small>현재 유효 자료 {insight.readiness.documentCount}건 · 발행기관 {insight.readiness.publisherCount}곳 · 공식/통계/재정 {insight.readiness.officialDocumentCount}건</small>
    </div> : <>
      <section className="dm-region-issues"><h3>근거로 확인한 지역 이슈</h3><div>{insight.issues.map((issue, index) => <article key={issue.id}>
        <span>0{index + 1}</span><div><strong>{issue.title}</strong><p>{issue.summary}</p>
          <small>{scopeLabel[issue.scope]} · 추세 {trendLabel[issue.trend]} · 점수 {issue.score} · 신뢰도 {issue.confidence === "high" ? "높음" : issue.confidence === "medium" ? "보통" : "낮음"} · 근거 {issue.evidenceIds.length}건</small>
          {issue.limitations.length > 0 && <p>한계: {issue.limitations.join(" · ")}</p>}
        </div>
      </article>)}</div></section>
      <section className="dm-region-angles"><h3>공고와 사업에 맞춘 제안 방향</h3><div>{insight.proposalAngles.map((angle, index) => <button className={angleId === angle.id ? "is-selected" : ""} type="button" key={angle.id} aria-pressed={angleId === angle.id} onClick={() => setAngleId(angle.id)}>
        <span>방향 {index + 1} · 적합도 {angle.fitScore}</span><strong>{angle.title}</strong><p>{angle.rationale}</p>
        <dl>
          <div><dt>지역 문제</dt><dd>{angle.regionalProblem}</dd></div>
          <div><dt>해결 방식</dt><dd>{angle.proposedSolution}</dd></div>
          <div><dt>공고 적합성</dt><dd>{angle.announcementFit}</dd></div>
          <div><dt>사업 적합성</dt><dd>{angle.businessFit}</dd></div>
          <div><dt>수혜 대상</dt><dd>{angle.beneficiaries.join(" · ")}</dd></div>
          <div><dt>실행 단계</dt><dd>{angle.executionPlan.join(" → ")}</dd></div>
    ...
    <section className="dm-region-evidence"><h3>분석 근거 자료</h3><div>{insight.evidence.map((evidence) => <a href={evidence.sourceUrl} target="_blank" rel="noreferrer" key={evidence.id}>
      <span>{sourceTypeLabel[evidence.sourceType] ?? evidence.sourceType} · {scopeLabel[evidence.scope]}</span>
    ...
    <footer><p>선택한 방향과 근거 ID만 제안서 작성 AI에 전달됩니다. 신청 자격과 원문 수치는 제출 전에 다시 확인하세요.</p><button className="dm-primary-button" type="button" disabled={!selected || blocked} onClick={() => selected && onWrite({ regionCode: insight.region.code, regionInsightId: insight.id, proposalAngleId: selected.id, announcementId: insight.announcement.id, regionName: insight.analysisUnit.name, proposalAngleTitle: selected.title }, announcement)}>이 방향으로 제안서 작성하기</button></footer>
  </section>;
}
