...
type OperationsPayload = {
  ...
  regionalEvidence: {
    ...
    units: Array<{ analysisUnitId: string; name: string; kind: string; dataState: "unconfigured" | "empty" | "insufficient" | "ready" | "error"; activeSourceCount: number; configuredSourceCount: number; documentCount: number; publisherCount: number; officialDocumentCount: number; recentDocumentCount: number; directDocumentCount: number; lastCollectedAt: string | null; lastError: string | null; lastSnapshotAt: string | null }>;
  };
...
function OperationsOverviewPanel({ payload, runAutomation, sendTestEmail }: { payload: OperationsPayload; runAutomation: () => Promise<void>; sendTestEmail: () => Promise<void> | void }) {
  return <div className="dm-operations-tab-content">
    ...
    <section className="dm-plan-section"><div className="dm-plan-section-head"><div><h2>지역 근거자료 46개 분석 단위</h2><p>수집원 0개는 준비 완료로 표시하지 않습니다. 단위별 실제 문서·발행기관·공식자료와 최근 오류를 확인합니다.</p></div><span className={payload.regionalEvidence.readyUnitCount === 46 ? "dm-runtime-badge is-openai" : "dm-runtime-badge"}>{payload.regionalEvidence.readyUnitCount}/{payload.regionalEvidence.supportedUnitCount} 준비</span></div><div className="dm-operation-grid">{payload.regionalEvidence.units.map((unit) => <div className="dm-operation-state" key={unit.analysisUnitId}><span>{unit.kind === "municipality" ? "경기 시·군" : "시·도 전체"} · {unit.analysisUnitId}</span><strong>{unit.name} · {unit.dataState === "ready" ? "운영 자료 준비됨" : unit.dataState === "unconfigured" ? "수집원 미연결" : unit.dataState === "empty" ? "수집 결과 없음" : unit.dataState === "error" ? "수집 오류" : "근거 부족"}</strong><small>활성 source {unit.activeSourceCount} · 문서 {unit.documentCount} · 발행기관 {unit.publisherCount}<br />공식/통계/재정 {unit.officialDocumentCount} · 최근 {unit.recentDocumentCount} · 직접 {unit.directDocumentCount}{unit.lastError ? <><br />최근 오류: {unit.lastError}</> : null}</small></div>)}</div></section>
